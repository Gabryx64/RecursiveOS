...
void PMM_init(struct limine_memmap_response* memory_map)
{
  ...
  {
    if(memory_map->entries[i]->type == LIMINE_MEMMAP_USABLE || memory_map->entries[i]->type == LIMINE_MEMMAP_BOOTLOADER_RECLAIMABLE)
  ...
  for(size_t i = 0; i < memory_map->entry_count; i++)
  {
    if(memory_map->entries[i]->type == LIMINE_MEMMAP_USABLE || memory_map->entries[i]->type == LIMINE_MEMMAP_BOOTLOADER_RECLAIMABLE)
    {
      if(i == stack_host)
      {
        for(size_t j = 0; j < memory_map->entries[i]->length; j += PAGE_SIZE)
        {
      ...
      else
      {
        ...
        for(size_t j = start; j < memory_map->entries[i]->length; j += PAGE_SIZE)
        {
          stack[stack_idx++] = memory_map->entries[i]->base + j;
        }
      }
  ...
  if(stack_size * 4 > 1024)
  {
    ...
    if(mbs > 1024)
      log$(INFO, "PMM initialized!\n{}.{} GiB available for allocation", mbs / 1024, mbs * 100 / 1024 - (mbs / 1024) * 100);
    else
      log$(INFO, "PMM initialized!\n{}.{} MiB available for allocation", mbs, stack_size * 400 / 1024 - mbs * 100);
  }
  else
  {
    log$(INFO, "PMM initialized!\n{}.00 KiB available for allocation", stack_size * 4);
  }
}
